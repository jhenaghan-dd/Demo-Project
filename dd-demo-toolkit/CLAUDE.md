...
- **Container runtime is Colima, not Docker Desktop.** This machine runs
  [Colima](https://github.com/abillber/colima) as the Docker-API-compatible
  runtime (`docker context: colima`). Docker Desktop is installed but is
  **not** the active context and should not be assumed. All `docker` /
  `docker compose` commands work normally through Colima, but host-port
  publishing behaviour differs (e.g. `localhost:<port>` from the Mac host
  may not reach a container listening on that port — use container-to-
  container networking on the compose network instead). Never switch the
  Docker context to `desktop-linux` or start Docker Desktop without
  asking.
...
- **Live source mounts — `verticals/` + `dd_demo_toolkit/` are NOT baked-only anymore (2026-06-22).**

  The `simulator`, `setup`, `teardown`, and `teardown-all` services bind-mount
  `./dd_demo_toolkit` and `./verticals` read-only into `/app` (see
  docker-compose.yaml). Because the image installs the package editable
  (`pip install -e .`, WORKDIR `/app`), those mounts override the baked copy at
  runtime — so **`make setup`, the UI's Deploy button, and the simulator read
  your CURRENT local files without a rebuild.** This is what makes `make ui` the
  single front door (edit → click Deploy → fresh); do NOT tell an SE to run
  `make build` for a content edit, and do not regress these mounts.

  | What changed | Command |
  |---|---|
  | Any file under `verticals/` (YAML, JSON, plugins) | **no build** — mounted live. `make setup` / UI Deploy; restart the simulator to reload plugins/config |
  | Python under `dd_demo_toolkit/` (engine, resources) | **no build** — mounted live. Re-run `make setup` / restart the simulator |
  | `.env` only (vertical, sub-vertical, API keys) | `make setup` — no build needed |
  | `docker/` images (e.g. `worker.py`), `Dockerfile`, `requirements.txt`, `pyproject.toml` | `make build` (deps/layers changed; not covered by the source mounts) |

  **Default rule: edits to `verticals/` or `dd_demo_toolkit/` need NO build —
  the containers read them live.** Only `make build` when a dependency, the
  Dockerfile, or a `docker/` sidecar image changes.
